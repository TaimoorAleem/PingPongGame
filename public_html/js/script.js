...
function update() {
  ...
  if (ball.y + ball.radius >= canvas.height || ball.y - ball.radius <= 0) {
   ...
   if (ball.x + ball.radius >= canvas.width) {
    ...
    user.score += 1;
    document.getElementById('h2').innerHTML = 'You scored one point! :)';
    reset();
  }
  ...
  if (ball.x - ball.radius <= 0) {
    ...
    ai.score += 1;
    document.getElementById('h2').innerHTML = 'AI scored one point! :(';
...
function render() {
  // set a style
  ctx.fillStyle = "blue"; /* whatever comes below this acquires black color (#000). */
...
// calls agameLoop() function 60 times per second
